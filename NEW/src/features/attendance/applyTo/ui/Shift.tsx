...
export const Shift = () => {
  ...
  const validateForm = () => {      
    const newErrors = {
          workDate: !form.wcwptchgreqDtoList[0].workDate,
          chgWorkPt: !form.wcwptchgreqDtoList[0].chgWorkPt,
          chgRsn: !form.wcwptchgreqDtoList[0].chgRsn,
    };
      // 상태 업데이트
  ...
  const handleDateSelect = (date: any) => {
    ...
    if ( date !== undefined && (Number(toDay.slice(-2)) - Number(date.slice(-2)) > 2) ) {
      ...
      form.wcwptchgreqDtoList[0].workDate = "";
    }else{
      setErrorDate(false);
     // handleSelectChange("wcwptchgreqDtoList[0].workDate", date)
      form.wcwptchgreqDtoList[0].workDate = date
    }
  }
  ...
  const [itemList, setItemList] =  useState([
    {label: "선택해주세요", error: false, query: ""},
        ...
        <UISelect ref={uiSelectRef}
          ...
          onQuerySelect={(value: any) => {
            ...
            if (shiftTarget[0].query === value) {
              alert("동일한 근무일정로 변경할 수 없습니다.");
              uiSelectReset()
              form.wcwptchgreqDtoList[0].chgWorkPt = "";
            }else if(value=== "-"){
              alert("휴무로는 변경할 수 없습니다.")
              uiSelectReset()            
              form.wcwptchgreqDtoList[0].chgWorkPt = "";
            } else {
              // handleSelectChange("wcwptchgreqDtoList[0].chgWorkPt", value);
              form.wcwptchgreqDtoList[0].chgWorkPt = value;
              setErrors((prevErrors) => ({ ...prevErrors, "chgWorkPt": false })); // Clear error on change
            }
          }}
          error={errors.chgWorkPt}
          hint={errors.chgWorkPt ? "변경후 근무일정을 확인해주세요." : ""}
          readOnly={fieldDisable}
        />
      </div>
      <div className="d-flex pt-10 pb-10">
        <UIInput label="변경사유" onChange={
          (e) => {
            form.wcwptchgreqDtoList[0].chgRsn = e.target.value
            setErrors((prevErrors) => ({ ...prevErrors, "chgRsn": false })); // Clear error on change
          }
        } 
        readOnly={fieldDisable}
        
          error={errors.chgRsn}
          hint={errors.chgRsn ? "변경사유를 입력해주세요" : ""}
        />
      </div>
