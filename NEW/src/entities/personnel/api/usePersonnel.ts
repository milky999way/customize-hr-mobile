...
import { PersonnelInfo, PersonnelInfoAcademic, PersonnelInfoDetail, PersonnelInfoItemCareer, PersonnelInfoItemFamily, PersonnelInquire } from "..";
...
// 식수 조회
const fetchPersonnelInquire = async (params: any): Promise<PersonnelInquire[]> => {
	const { data } = await axiosInstance.get(`/one/dbhempmeal/empmeal100?fromMonth=${params.fromMonth}&toMonth=${params.toMonth}`);
	return data;
};
export const usePersonnelInquire = (params: any) => {
	return useQuery<PersonnelInquire[], Error>({
		queryKey: ['PersonnelInquire', params],
		queryFn: () => fetchPersonnelInquire(params),
		initialData: [],
		enabled: !!params.fromMonth && !!params.toMonth
	});
};