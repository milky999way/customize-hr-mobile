...
const fetchAttendanceScheduleMember = async (params: any): Promise<AttendanceScheduleMember[]> => {
	const { data } = await axiosInstance.get(`/wrk/wrkleav/wrkcalendar130?searchOrgCode=${params.searchOrgCode}&searchOrgNameHan=${params.searchOrgNameHan}&baseMonth=${params.baseMonth}`);
	return data;
};
export const useAttendanceScheduleMember = (params: any) => {
	return useQuery<AttendanceScheduleMember[], Error>({
		...
		initialData: [],
		enabled: !!params.searchOrgCode && !!params.searchOrgNameHan && !!params.baseMonth
	});
};
...
const fetchAttendanceOverTime = async (params: any): Promise<AttendanceOverTime[]> => {
	const { data } = await axiosInstance.get(`/uhr/docappr/approvtm200/findWorkCode?emplNo=${params.emplNo}&workDate=${params.workDate}&befDay=${params.befDay}`);
	return data;
};
