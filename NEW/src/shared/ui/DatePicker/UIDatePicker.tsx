...
type UIDatePickerProps = {
    placeholder?: string;
    label?: string;
    type?: "date" | "range" | "month" | "year-month" | "year-month-choose";
    error?: boolean;
    disabled?: boolean;
    readOnly?: boolean;
    hint?: string;
    ...
    viewSelectedDate?: (month: Date) => void;
};
...
export const UIDatePicker = forwardRef<any, UIDatePickerProps>(({ 
    placeholder = "날짜를 선택해주세요", label, hint, type = "date", error = false, disabled = false, readOnly = false, onDateSelect, onDateRangeChange, onMonthSelect 
}, ref) => {
    ...
    const viewSelectedDate = (viewDate:any) => {
        setViewDate(viewDate)  
        setSelectedYear(viewDate.split("-")[0]);  
        setSelectedMonthGrid(Number(viewDate.split("-")[1])-1);      
    };
            ...
            {type === "range" ? (
                <div className="datepicker__range__trigger">
                    ...
                    <Popover open={isStartOpen} onOpenChange={setIsStartOpen}>
                        ...
                        <PopoverContent className="range datepicker__content d-flex justify-content-end align-items-center flex-direction-column ">
                            <DayPicker
                                mode="range"
                                locale={ko}
                    ...
                    <PopoverTrigger asChild>
                        ...
                        >
                            {selectedYear && selectedMonthGrid !== undefined
                                ? selectedYear + "-" + (Number(selectedMonthGrid) + 1).toString().padStart(2, "0") + ""
                                : placeholder}
                            <div className={`datepicker__icon icon is-calender d-block ml-10 ${disabled ? "is-disabled" : ""}`}></div>
                        </button>
                    </PopoverTrigger>
