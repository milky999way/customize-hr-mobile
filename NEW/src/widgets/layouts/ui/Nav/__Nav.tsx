...
export const Navigation = () => {
	...
	// const { data: menuData, isLoading: isMenuLoading, error: menueError } = useMenu();
	// if (isMenuLoading) return <p>Loading...</p>;
	// if (menueError) return <p>Error: {menueError.message}</p>;
	// console.log(data);
	// const menu = menuData.menuList.map((i) => { return i.mobileUseAt })
	// const menu = menuData.menuList ? menuData.menuList.map((i) => {return i.mobileUseAt}) : null;
	// console.log(menu)
	...
	return (
		<NavigationMenu className={`${isNavOpen ? "Root open" : "Root"}`}>
			...
			<div className="inner">
				<div className="User">
					<div>
						<UIAvatar size="large" src={'http://hrms.dbhitekdev.com/files/'+userData.photoFileId+'/'+userData.fileSn+'/download'} fallback="이미지 로드 실패" />
				...
				<div className="Menu">
					<NavigationMenuList className="MenuList">
						...
						<NavigationMenuItem>
							...
							<NavigationMenuList>
								<NavigationMenuItem>
									<Link to="/attendance/apply-list" onClick={toggleNav}>근태 신청 현황</Link>
								</NavigationMenuItem>
								<NavigationMenuItem>
								...
								<NavigationMenuItem>
									<Link to="/attendance/chooseplan" onClick={toggleNav}>월간 선택근로 신청</Link>
								</NavigationMenuItem>
								<NavigationMenuItem>
						...
						{/* <NavigationMenuItem>
							...
							<NavigationMenuList>
								...
								<NavigationMenuItem>
									<Link to="/tenure/leave-info" onClick={toggleNav}>퇴사FLOW 목록</Link>
								</NavigationMenuItem>
								<NavigationMenuItem>
									<Link to="/tenure/leave-flow" onClick={toggleNav}>퇴사FLOW 입력</Link>
								</NavigationMenuItem>
								<NavigationMenuItem>
